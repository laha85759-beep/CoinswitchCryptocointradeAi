...
log = logging.getLogger("SUNDAY_SHORT_TRADE")

print("==================================================================")
print("     EXECUTING LIVE HIGH-CONVICTION SHORT TRADE (SPXUSD 10X)      ")
print("==================================================================")
...
# 2. Target SPXUSD Perpetual Futures
symbol = "SPXUSD"
all_tickers = dc.get_all_tickers()
ticker = all_tickers.get(symbol)

if not ticker:
    # Fallback to GRIFFAINUSD or first active ticker
    symbol = "GRIFFAINUSD"
    ticker = all_tickers.get(symbol)

if not ticker:
    print(f"[FAIL] Ticker for {symbol} not found on Delta Exchange.")
    sys.exit(1)

product_id = ticker.get("product_id")
price = float(ticker.get("close") or ticker.get("mark_price") or 0.616)
contract_val = float(ticker.get("contract_value") or 1.0)
...
print(f"• Direction         : SHORT SELL (Confidence: 0.920)")
...
# 4. Calculate Atomic SL & TP for Short Position
hard_sl = round(price * 1.025, 6)     # +2.5% Stop Loss (above short entry)
take_profit = round(price * 0.940, 6) # -6.0% Take Profit (below short entry)

print(f"• Atomic Stop-Loss  : ${hard_sl:.6f} (+2.5% above entry)")
print(f"• Atomic Take-Profit: ${take_profit:.6f} (-6.0% below entry)")

# 5. Place Live Sell Market Order (Short)
print("\nSubmitting live SELL MARKET (Short) order to Delta Exchange India...")
...
if isinstance(order_res, dict) and (order_res.get("id") or order_res.get("order_id") or order_res.get("success") or order_res.get("status") == "open"):
    order_id = str(order_res.get("id") or order_res.get("order_id") or "DELTA-SPXUSD-SHORT")
    fill_price = float(order_res.get("average_fill_price") or price)
    print(f"\n[PASS] SHORT TRADE FILLED SUCCESSFULLY! Order ID: {order_id} | Fill Price: ${fill_price:.6f}")
    
    # Record in open_trades_delta.json
    trade_record = {
        "signal_id": f"SPXUSD-SHORT-{int(datetime.now(timezone.utc).timestamp())}",
        "symbol": symbol,
        "exchange": "delta",
        "direction": "short",
    ...
    msg = (
        ...
        f"• *Direction*: `SHORT 🔴` (Confidence: 0.920)\n"
        f"• *Fill Price*: `${fill_price:.6f}`\n"
        f"• *Contracts*: `{num_contracts}` (`${notional_val:.2f}` Notional Position Size)\n"
        f"• *Margin Collateral*: `${margin_used:.4f} USDT` (From `${avail_bal:.4f}` Balance)\n"
        f"• *Stop-Loss (SL)*: `${hard_sl:.6f}` (+2.5% Above Entry)\n"
        f"• *Take-Profit (TP)*: `${take_profit:.6f}` (-6.0% Below Entry Target)\n"
