...
class TelegramDiagnosticExplainerAgent:
    ...
    def run_audit_and_notify_telegram(self) -> Dict[str, Any]:
        ...
        msg = (
            f"🩺 *OPUS 4.7 • 24/7 QUANT SYSTEM DIAGNOSTIC*\n"
            f"═════════════════════════\n"
            f"🕒 *Timestamp*: `{now_utc}`\n"
            f"📊 *Account Capital*:\n"
            f"  • *Delta Exchange*: `${delta_usdt:.4f} USDT` 🟢\n"
            f"  • *CoinSwitch Pro*: `₹{cs_inr:.2f} INR` 🟢\n"
            f"  • *Active Positions*: `{active_pos_count}` Open\n"
            f"─────────────────────────\n"
            f"🔍 *QUANT PIPELINE DIAGNOSTIC*:\n"
            f"📌 *State*: *{reason_title}*\n"
            f"📝 *Details*: {reason_detail}\n"
            f"─────────────────────────\n"
            f"🚀 *TOP SCANNED CANDIDATES*:\n"
            f"{top_candidates_str}"
            f"─────────────────────────\n"
            f"🛡️ *SYSTEM HEALTH CHECK*:\n"
            f"• *Exchange APIs*: `ONLINE [PASS]`\n"
            f"• *Quant Engine*: `OPERATIONAL [PASS]`\n"
            f"• *AI Reasoner*: `NVIDIA NEMOTRON 550B [PASS]`\n"
            f"• *24/7 Daemons*: `CONTINUOUS DAEMON RUNNING [PASS]`\n"
            f"═════════════════════════\n"
