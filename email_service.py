...
<head>
  ...
  <style>
    ...
    .container { max-width:580px; margin:30px auto; background:#060e1d; border:1px solid #00f090; border-radius:10px; overflow:hidden; box-shadow:0 10px 40px rgba(0,0,0,0.8); }
    .header { background:linear-gradient(135deg,#091a33,#020812); padding:24px; text-align:center; border-bottom:1px solid rgba(0,240,144,0.3); }
    .header h1 { margin:0; color:#00f090; font-size:20px; font-weight:800; letter-spacing:1.5px; }
    .content { padding:30px 24px; line-height:1.6; font-size:14.5px; color:#cbd5e1; }
    ...
    .btn { display:inline-block; background:linear-gradient(135deg,#00f090,#00a8ff); color:#020812 !important; text-decoration:none; padding:12px 28px; border-radius:6px; font-weight:800; font-size:13px; text-transform:uppercase; margin:15px 0; }
    .footer { background:#02060d; padding:20px; text-align:center; font-size:11.5px; color:#64748b; border-top:1px solid rgba(255,255,255,0.08); }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <div style="font-size:11px; color:#00d4ff; margin-top:4px;">Institutional Multi-Exchange Trading Engine</div>
    </div>
    <div class="content">
      ''' + content_html + '''
    </div>
    <div class="footer">
      <p>&copy; 2026 TheSmartMag Quant Network &bull; <a href="https://trade.thesmartmag.com">trade.thesmartmag.com</a></p>
      <p>Direct Inquiries: <a href="mailto:''' + sender_footer + '''">''' + sender_footer + '''</a></p>
      <p style="font-size:10px; color:#475569;">This is an automated system dispatch. Confidential & Encrypted.</p>
    </div>
  </div>
</body>
</html>'''
...
def send_welcome_email(to_email, name, role='trader', phone='', country='US', preferred_exchange='both'):
    ...
    content = '<p>Greetings <strong>' + str(name) + '</strong>,</p>' + \
      ...
      '<div style="background:rgba(2,8,18,0.6); border:1px solid rgba(0,240,144,0.3); border-radius:6px; padding:14px 18px; margin:20px 0;">' + \
      ...
      '<p style="font-size:12.5px; color:#94a3b8;">Need technical assistance or broker connectivity setup? Reach out to our dedicated support desk at <a href="mailto:support@thesmartmag.com" style="color:#00d4ff;">support@thesmartmag.com</a>.</p>'
...
def send_password_reset_email(to_email, name, otp_code, reset_token):
    ...
    content = '<p>Greetings <strong>' + str(name or 'Trader') + '</strong>,</p>' + \
      ...
      '<p style="font-size:12px; color:#94a3b8;">For security questions, contact <a href="mailto:support@thesmartmag.com" style="color:#00d4ff;">support@thesmartmag.com</a>.</p>'
...
def send_inquiry_confirmation(to_email, name, subject_topic, sender_type='query'):
    ...
    content = '<p>Hello <strong>' + str(name) + '</strong>,</p>' + \
      ...
      '<div style="background:rgba(2,8,18,0.6); border:1px solid rgba(0,212,255,0.3); border-radius:6px; padding:12px 16px; margin:18px 0; font-size:13px;">' + \
      '<div><strong>Assigned Desk:</strong> ' + str(sender_addr) + '</div>' + \
      '<div><strong>Status:</strong> Active Ticket In Queue</div>' + \
      '</div>' + \
      '<p style="font-size:12.5px; color:#94a3b8;">For immediate trading questions, visit <a href="https://trade.thesmartmag.com" style="color:#00d4ff;">trade.thesmartmag.com</a>.</p>'
