...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // RWA matrix live prices
  fetchRwaLivePrices();
  setInterval(fetchRwaLivePrices, 60000);
  // XAUUSD/NASDAQ/SP500
  fetchXAUUSDPrice();
  setInterval(fetchXAUUSDPrice, 30000);
  fetchStockIndices();
...
// ── XAUUSD Live Price (CoinGecko: gold) ───────────────────────────────────
async function fetchXAUUSDPrice() {
  try {
    const res = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=tether-gold,gold&vs_currencies=usd', {cache:'no-store'});
    if (res.ok) {
      const d = await res.json();
      const goldPrice = d?.gold?.usd || d?.['tether-gold']?.usd;
      if (goldPrice) {
        const fmtPrice = '$' + Number(goldPrice).toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2});
        ['tick-xauusd', 'hero-xauusd-price', 'rwa-p-xauusd', 'rwa-p-xaut'].forEach(id => {
...
// ── NASDAQ & S&P 500 via AllOrigins proxy ────────────────────────────────
async function fetchStockIndices() {
  const symbols = [
    {sym:'QQQ',  nasdaqId:'rwa-p-nasdaq', tickId:'tick-nasdaq', tickChgId:'tick-nasdaq-chg'},
    {sym:'SPY',  nasdaqId:'rwa-p-sp500',  tickId:'tick-sp500',  tickChgId:'tick-sp500-chg'}
  ];
  for (const s of symbols) {
    try {
      const url = `https://query1.finance.yahoo.com/v8/finance/chart/${s.sym}?interval=1d&range=1d`;
      ...
      if (r.ok) {
        ...
        if (price) {
          ...
          [s.nasdaqId, s.tickId].forEach(id => { const el = document.getElementById(id); if (el) el.textContent = fmt + ' '; });
          if (prevClose) {
            const pct = ((price - prevClose) / prevClose * 100).toFixed(2);
            const sign = pct >= 0 ? '+' : '';
            [s.tickChgId].forEach(id => {
              const el = document.getElementById(id);
              if (el) { el.textContent = sign + pct + '%'; el.style.color = pct >= 0 ? 'var(--neon-green, #00ff88)' : 'var(--neon-pink, #ff3366)'; }
            });
          }
        }
      }
...
window.fetchStockIndices = fetchStockIndices;

// ── RWA Matrix: Live Prices for all 30+ coins ─────────────────────────────
const RWA_COIN_IDS = {
  'ondo': 'ondo-finance', 'om': 'mantra-dao', 'pendle': 'pendle', 'link': 'chainlink',
  'avax': 'avalanche-2', 'mkr': 'maker', 'cfg': 'centrifuge', 'xaut': 'tether-gold',
  'sol': 'solana', 'sui': 'sui', 'apt': 'aptos', 'tia': 'celestia', 'inj': 'injective-protocol',
  'arb': 'arbitrum', 'op': 'optimism', 'jup': 'jupiter-exchange-solana', 'doge': 'dogecoin',
  'shib': 'shiba-inu', 'pepe': 'pepe', 'wif': 'dogwifcoin', 'bnb': 'binancecoin',
  'ada': 'cardano', 'dot': 'polkadot', 'atom': 'cosmos', 'near': 'near', 'ftm': 'fantom',
  'uni': 'uniswap', 'aave': 'aave', 'crv': 'curve-dao-token'
};

async function fetchRwaLivePrices() {
  try {
    const ids = Object.values(RWA_COIN_IDS).join(',');
    ...
    for (const [ticker, cgId] of Object.entries(RWA_COIN_IDS)) {
      const data = prices[cgId];
      if (data?.usd) {
        const el = document.getElementById(`rwa-p-${ticker}`);
        if (el) {
          const p = Number(data.usd);
          const fmtP = p < 0.01 ? '$' + p.toFixed(8) : p < 1 ? '$' + p.toFixed(4) : '$' + p.toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2});
          const chg = data.usd_24h_change || 0;
          const chgStr = (chg >= 0 ? '+' : '') + chg.toFixed(2) + '%';
          el.innerHTML = `${fmtP} <span style="font-size:10px; color:${chg >= 0 ? 'var(--neon-green)' : 'var(--neon-pink)'}">${chgStr}</span>`;
        }
      }
    }
    const lastEl = document.getElementById('rwaLastUpdate');
    if (lastEl) lastEl.textContent = 'Last: ' + new Date().toLocaleTimeString();
  } catch(e) {}
}
window.fetchRwaLivePrices = fetchRwaLivePrices;
window.refreshRwaMatrix = fetchRwaLivePrices;

// ── Share RWA snapshot to Telegram ────────────────────────────────────────
async function shareRwaToTelegram() {
  try {
    const res = await fetch('/api/telegram/share-rwa-snapshot', {
      method: 'POST',
      headers: {'Authorization': `Bearer ${userToken}`, 'Content-Type': 'application/json'},
      body: JSON.stringify({message_type: 'rwa_matrix'})
    });
    const d = await res.json();
    alert(d.status === 'success' ? '📲 RWA Matrix shared to Telegram!' : 'Telegram: ' + (d.message || 'Failed'));
  } catch(e) {
    alert('Telegram share failed: ' + e.message);
  }
}
window.shareRwaToTelegram = shareRwaToTelegram;

// ── CSS utility classes check & inject if missing ─────────────────────────
(function injectMissingCSS() {
  const style = document.createElement('style');
  style.id = 'tsm-runtime-css';
  style.textContent = `
    .tsm-rwa-price { font-family: var(--font-mono, monospace); font-size: 11px; color: var(--neon-green, #00ff88); margin: 2px 0; font-weight: 700; }
    .tsm-filter-btn { padding: 5px 12px; background: rgba(0,240,144,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: 4px; color: var(--text-dim, #6e7681); font-family: var(--font-mono, monospace); font-size: 10px; letter-spacing: 1px; cursor: pointer; transition: all 0.2s; }
    .tsm-filter-btn:hover, .tsm-filter-btn.active { background: rgba(0,240,144,0.15); border-color: var(--neon-green, #00ff88); color: var(--neon-green, #00ff88); }
    .rwa-loading-msg { color: var(--text-dim, #6e7681); font-family: var(--font-mono, monospace); font-size: 11px; padding: 16px; }
    .strat-showcase-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 16px; margin-bottom: 20px; }
    .strat-card { background: rgba(0,0,0,0.4); border: 1px solid rgba(255,255,255,0.07); border-radius: 10px; padding: 18px; display: flex; flex-direction: column; justify-content: space-between; gap: 12px; transition: all 0.3s; }
    .strat-card:hover { border-color: rgba(0,240,144,0.3); box-shadow: 0 0 24px rgba(0,240,144,0.08); transform: translateY(-2px); }
    .strat-card.active-strat { border-color: rgba(0,240,144,0.5); box-shadow: 0 0 30px rgba(0,240,144,0.15); }
    .strat-winrate-tag { font-family: var(--font-orb, sans-serif); font-size: 28px; font-weight: 800; color: var(--neon-green, #00ff88); margin: 10px 0 4px; text-shadow: 0 0 20px rgba(0,240,144,0.4); }
    .strat-metrics-row { display: flex; gap: 12px; margin-top: 8px; }
    .strat-metrics-row > div { flex: 1; text-align: center; padding: 8px; background: rgba(0,0,0,0.3); border-radius: 6px; }
    .m-val { font-family: var(--font-orb, sans-serif); font-size: 14px; font-weight: 700; }
    .m-lbl { font-family: var(--font-mono, monospace); font-size: 9px; color: var(--text-dim, #6e7681); letter-spacing: 1px; margin-top: 2px; }
    .custom-strat-panel { background: rgba(0,0,0,0.3); border: 1px solid rgba(0,240,144,0.15); border-radius: 10px; padding: 20px; margin-top: 4px; }
    .ind-checkbox-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 8px; margin-top: 8px; }
    .ind-checkbox-label { display: flex; align-items: center; gap: 8px; padding: 8px 12px; background: rgba(0,240,144,0.04); border: 1px solid rgba(255,255,255,0.06); border-radius: 6px; cursor: pointer; font-size: 11.5px; color: var(--text-sec, #8ab4cf); transition: all 0.2s; }
    .ind-checkbox-label:hover { background: rgba(0,240,144,0.08); border-color: rgba(0,240,144,0.3); }
    .ind-checkbox-label input { accent-color: var(--neon-green, #00ff88); }
  `;
...
async function fetchTraderTerminalData() {
  if (!currentUser || !userToken) return;
  try {
    const res = await fetch("/api/user/trader-dashboard", {
      headers: { "Authorization": `Bearer ${userToken}` }
    });
    if (!res.ok) return;
    const data = await res.json();
    if (data.status === "success") {
      renderTraderDashboard(data, null);
    }
  } catch(e) {
    console.debug("fetchTraderTerminalData error:", e);
  }
}
window.fetchTraderTerminalData = fetchTraderTerminalData;
...
function renderTraderDashboard(userData, fullData) {
  ...
  // Only use user's own positions — NEVER fall back to bot/admin positions
  const effectiveOpenPos = openPos || { coinswitch: [], delta: [] };
...
async function fetchRealData() {
  try {
    ...
    if (data.tickers) {
      ...
      // Update XAUUSD ticker strip from server data (CoinGecko will also update it)
      const xauusdPrice = data.tickers.xauusd || data.tickers.gold || data.tickers.xau;
      if (xauusdPrice) {
        const xauEl = document.getElementById('tick-xauusd');
        if (xauEl) xauEl.textContent = '$' + Number(xauusdPrice).toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2});
      }
